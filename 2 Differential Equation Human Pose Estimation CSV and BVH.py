...
cap = cv2.VideoCapture('test.mp4')  # Update with the path to the video file
frame_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
frame_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
output_annotated = cv2.VideoWriter('DE_HPS_output_annotated.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 20, (frame_width, frame_height))
output_skeleton = cv2.VideoWriter('DE_HPS_output_skeleton.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 20, (frame_width, frame_height))
...
# Define base offsets for each joint relative to its parent in the base bind pose
initial_offsets = {
    'Nose': np.array([0, 0, 0]),
...
# Objective function for DE: minimize the error between detected and optimized joint positions
def objective_function(params, joint_positions, joint_name):
    optimized_pos = np.array(params)
    actual_pos = joint_positions[joint_name]
    error = np.linalg.norm(actual_pos - optimized_pos) ** 2  # Squared error between actual and optimized positions
    return error

# apply DE on each joint position with reduced population size and fewer iterations
def optimize_joint_position(joint_positions, joint_name):
    bounds = [(0, 1)] * 3  # Bounds for x, y, z for each joint
    result = differential_evolution(objective_function, bounds, args=(joint_positions, joint_name), maxiter=5, popsize=5)
    return result.x  # Optimized (x, y, z)
...
with open('DE_HPS_joint_data_with_rotation_and_offset.csv', 'w', newline='') as csv_file:
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(['Frame', 'Joint', 'X', 'Y', 'Z', 'OffsetX', 'OffsetY', 'OffsetZ', 'Pitch', 'Yaw', 'Roll'])

    with open('DE_HPS.bvh', 'w') as bvh_file:
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                print(f"Optimizing joints for Frame {frame_count}")

                # Apply DE to optimize joint positions
                optimized_joint_positions = {}
                for joint_name in joint_names:
                    optimized_pos = optimize_joint_position(joint_positions, joint_name)
                    optimized_joint_positions[joint_name] = optimized_pos

                print(f"Drawing landmarks for Frame {frame_count}")

                # Draw pose landmarks on the annotated image using optimized positions
                ...
                for joint, parent_joint in hierarchy.items():
                    if parent_joint in optimized_joint_positions:
                        parent_pos = optimized_joint_positions[parent_joint]
                        child_pos = optimized_joint_positions[joint]
                    ...
                    # Get the optimized coordinates of the current joint
                    x, y, z = optimized_joint_positions[joint]
                ...
                print(f"Finished processing Frame {frame_count}")
